...
export const GDriveCinematicBanner = ({
  ...
  const initialSource = resolveVideoSource(src ?? GDRIVE_BANNER_SRC);
  const [videoSrc, setVideoSrc] = useState(initialSource);

  useEffect(() => {
    setVideoSrc(initialSource);
  }, [initialSource]);
  ...
  return (
    <section className={`container mx-auto px-4 lg:px-8 ${className}`}>
      ...
      >
          ...
          src={videoSrc}
          onError={() => {
            if (videoSrc !== "/actualites-banner.mp4") {
              setVideoSrc("/actualites-banner.mp4");
            }
          }}
