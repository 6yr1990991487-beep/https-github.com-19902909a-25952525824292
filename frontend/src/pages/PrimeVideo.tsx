...
const PrimeVideo = () => {
  ...
  // Load synced YouTube/Prime-compatible videos from the FastAPI/MongoDB auto-sync layer.
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
        ...
        const rows = (json.videos ?? [])
          .filter((r: any) => r.external_id || r.id)
          .map((r: any): Item => ({
            id: r.id ?? r.external_id,
            videoId: r.external_id ?? r.id,
            title: r.title ?? "Anime.Moments.officiel",
            thumbnail: r.thumbnail_url || r.thumbnail || ytThumb(r.external_id ?? r.id),
          }));
        ...
        if (!alive) return;
        setItems(fallback);
        setActive((curr) => curr || fallback[0]?.videoId || "");
      }
    })();
    return () => {
      alive = false;
